...
func (t *gcsFuseCSIOIDCTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	ginkgo.It("should successfully mount with OIDC authentication", ginkgo.Serial, func() {
		enableWIFEnforcement()
		testCaseOIDCMount()
	})

	ginkgo.It("should store and retain data with OIDC authentication", ginkgo.Serial, func() {
		enableWIFEnforcement()
		testCaseOIDCStoreData()
	})

	ginkgo.It("should store data in implicit directory with OIDC authentication", ginkgo.Serial, func() {
		enableWIFEnforcement()
		testCaseOIDCStoreDataInImplicitDir()
	})

	ginkgo.It("should fail when OIDC ConfigMap is missing", ginkgo.Serial, func() {
		enableWIFEnforcement()
		testCaseOIDCMissingConfigMap()
	})

	ginkgo.It("should fail when CSI bucket access check is enabled with OIDC authentication", ginkgo.Serial, func() {
		enableWIFEnforcement()
	...
	ginkgo.It("should fail to start pod when WIF credential config is absent and node SA has bucket access", ginkgo.Serial, func() {
		enableWIFEnforcement()
